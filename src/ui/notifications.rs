...
pub fn render_notifications(
    ...
    if let Some(n) = state.selected_notification() {
        ...
        let detail = vec![
            ...
            Line::from(vec![
                Span::styled("原因: ", Style::default().fg(Color::Cyan)),
                Span::raw(&n.reason),
            ]),
            Line::from(vec![
                Span::styled("时间: ", Style::default().fg(Color::Cyan)),
                Span::raw(&time_str),
            ]),
            Line::from(""),
...
pub fn handle_notification_input(
    ...
    match key {
        ...
        crossterm::event::KeyCode::Up | crossterm::event::KeyCode::Char('k') => {
            let i = state.list_selected.map_or(0, |i| {
                if i == 0 {
                    state.notifications.len().saturating_sub(1)
                } else {
                    i - 1
                }
            });
        ...
        crossterm::event::KeyCode::Down | crossterm::event::KeyCode::Char('j') => {
            let i = state.list_selected.map_or(0, |i| {
                if i >= state.notifications.len().saturating_sub(1) {
                    0
                } else {
                    i + 1
                }
